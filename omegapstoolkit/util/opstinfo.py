...
#---[Metadata]--------------------------------------------------------------#
#  Filename ~ opsthelp.py                 [Update:  2022-05-30 | 1:40  PM]  #
#---[Info]------------------------------------------------------------------#
...
def opstinfo():
    print(f"""
{sc.GR}{sc.D} ______  ______ _______ _______ {sc.W}{sc.G}  OmegaPSToolkit Inforomation {sc.D}version: {__VERSION_OPSTINFO}{sc.W}
{sc.GR}{sc.D}|       |   __ \     __|_     _|{sc.G}{sc.D}  Coded by MyMeepSQL for © PSociety™{sc.W}
{sc.GR}{sc.D}|   -   |    __/__     | |   |  {sc.W}{sc.D}  A massive penetration testing toolkit{sc.W}
...
{sc.C}Informations about OmegaPSToolkit{sc.GR}:{sc.W}
    ...
    {sc.G}All OPST commands versions{sc.GR}:{sc.W}
        opstconsole                {__VERSION_OPSTCONSOLE}
        odstconsole CLI            {__VERSION_OPSTCONSOLE_CLI}
        opstupdate                 {__VERSION_OPSTUPDATE}
        opstinfo                   {__VERSION_OPSTINFO}
        odstsetup                  {__VERSION_OPSTSETUP}
        odstinstall-all            {__VERSION_OPSTINSTALL}
...
{sc.C}Ohter informations{sc.GR}:{sc.W}
    ...
    {sc.G}Network{sc.GR}:{sc.W}
        ...
        MAC adress ({sc.C}eth0{sc.W})          {sc.O}{mac_adress}{sc.W}
    ...
    for gpu in gpus:
        try:
            print(f"""            Card {sc.C}{GPU_details_id}{sc.W}:
